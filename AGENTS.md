...
- **Swarm stack not yet wired for artifact delivery:** the registrator side is built — `INFRA_REGISTRATOR_RELOAD_MODE` (auto/artifact), `auto` restart with `krakend check` + atomic promote, `artifact` one-shot delivery (render → per-service `info.version` gate → immutable Swarm config object `krakend-config-<hash>` → `UpdateServiceConfig`), and `-once`/`-dry-run`. Verified incl. live single-node swarm tests. **Remaining:** `docker-compose.swarm.yml` completeness (one-shot `logto-init`, TLS on :443, Docker secrets incl. M2M, resource limits, KrakenD config-object wiring + `order:start-first`), and the CI scripting (`stack deploy --detach=false` convergence wait → `registrator -once`; multi-repo CI-to-CI trigger). `krakend check` in artifact mode is a CI step (`-dry-run | krakend check`), not in-process.
