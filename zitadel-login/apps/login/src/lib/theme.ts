...
export const SPACING_STYLES = {
  // FORK: regular padding is admin's --d-card-pad (24px) on all sides; upstream
  // added extra vertical padding, which made the card look taller than admin's.
  regular: {
    spacing: "space-y-6",
    padding: "p-6",
  },
...
export const APPEARANCE_STYLES = {
  // FORK: "flat" is our appearance — retuned to admin's card/control chrome
  // (components.css .card / .btn--outline): hairline --ui-border instead of
  // black/10, admin's --shadow-xs, medium type. Surfaces stay branding-driven.
  flat: {
    card: "bg-background-light-400 dark:bg-background-dark-500 border border-[var(--ui-border)] shadow-[var(--ui-shadow-xs)]",
    button: "border border-[var(--ui-border)]",
    "idp-button": "border border-[var(--ui-border)] hover:bg-[var(--ui-accent)]",
