...
export const ThemeWrapper = ({ children, branding }: Props) => {
  ...
  // FORK: this wrapper sits between the page shell and the card, so it has to
  // pass the shell's height down — otherwise the full-height mobile layout
  // (see dynamic-theme.tsx) has its flex chain broken here and collapses back
  // to a band floating in the middle of the screen.
  return <div className="flex flex-1 flex-col sm:block sm:flex-none">{children}</div>;
};
