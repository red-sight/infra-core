...
function getSelectedButtonStyle(isSelected: boolean): string {
  ...
  // FORK: admin's segmented-control look — muted glyphs, selected chip on the
  // card surface with a hairline border, instead of raw gray-200/gray-800.
  if (!isSelected) {
    return "text-[var(--ui-muted-foreground)] hover:text-text-light-500 dark:hover:text-text-dark-500";
  }
  ...
  switch (themeConfig.appearance) {
    ...
    case "flat":
    default:
      return "bg-background-light-400 dark:bg-background-dark-400 text-text-light-500 dark:text-text-dark-500 border border-[var(--ui-border)]";
  }
}
